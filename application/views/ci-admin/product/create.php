...
<form class="form-create-product" method="post" action="<?php echo base_url('ci-admin/product/create') ?>">
   ...
   <div class="form-group col-md-4 form-select">
      <label for="idSubChild">idSubChild:</label>
      <select class="form-control" name="idSubChild" id="idSubChild">
        <?php foreach($dataMenu as $key=>$value): ?>
             <?php if(count($value['sub']) >=1): ?>
                <option value="0">Mời bạn chon Menu</option>
   ...
   <div class="form-group col-md-4 form-select">
      <label for="idChild">idChild:</label>
      <select class="form-control" name="idChild" id="idChild">
         <?php foreach($dataMenu as $key=>$value): ?>
             <?php if(count($value['sub']) >=1): ?>
               <?php foreach($value['sub'] as $key1 => $value1): ?>
